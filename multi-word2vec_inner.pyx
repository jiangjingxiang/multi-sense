...
def train_sentence_sg(model, sentence, alpha, _work):
    ...
    # For center calculate
    cdef REAL_t *average_context
    cdef int context_count
    cdef int sense_no
    cdef REAL_t *syn1center
    cdef REAL_t *syn1center_count
    cdef string word_str
    cdef string *index2word
    ...
    # =========jjx=============
    syn1center = <REAL_t *>(np.PyArray_DATA(model.syn1center))
    syn1center_count = <REAL_t *>(np.PyArray_DATA(model.syn1center_count))
    index2word = <string *>(np.PyArray_DATA(model.index2word))
    ...
    with nogil:
        for i in range(sentence_len):
            ...
            if k > sentence_len:
                k = sentence_len
            #========jjx==center======
            memset(average_context, 0, size * cython.sizeof(REAL_t))
            context_count = 0
            for context_index in range(j, k):
                if context_index == i or codelens[context_index] == 0:
                    continue
                row = indexes[context_index] * size
                average_context += syn1neg[row]
                context_count += 1
            average_context /= context_count
            #=============get_sense_no======================

            word_str = index2word[indexes[i]]
            for sense_index in range(MAX_SENSE):
                word_sense_str = word_str + "#" + str(sense_index)
                if model.vocab_sense.get(word_sense_str):
                    word_node = model.vocab_sense[word_sense_str]
                    node_index = word_node.index
                    temp_center = syn1center[node_index * size]

            #sense_no = get_sense_no(model, word_str, average_context)
            #update(model, indexes[i], sense_no, average_context)
...
#cdef int get_sense_no(REAL_t *syn1center, REAL_t *syn1center_count, string word_str, REAL_t *average_context) nogil:
#    cdef REAL_t *temp_center
#    for i in range(MAX_SENSE):
